 ...
 * @author developer
 */
public class SubscriptionSummaryReportSortBySubErrorNoDrTrue implements Comparator<SummaryReport> {
    public int compare(SummaryReport s1, SummaryReport s2) {
        if (s1==null || s2==null) {
            return 0;
        }
        return s1.getSub_error_nodr(CARRIER.TRUE) - s2.getSub_error_nodr(CARRIER.TRUE);
    }
}
