 ...
 * @author developer
 */
public class SubscriptionSummaryReportSortBySubFtAis implements Comparator<SummaryReport> {
    public int compare(SummaryReport s1, SummaryReport s2) {
        if (s1==null || s2==null) {
            return 0;
        }
        return (s1.getSub_ft(CARRIER.AIS) + s1.getSub_ft(CARRIER.AIS_LEGACY)) -
                (s2.getSub_ft(CARRIER.AIS) + s2.getSub_ft(CARRIER.AIS_LEGACY));
    }
}
