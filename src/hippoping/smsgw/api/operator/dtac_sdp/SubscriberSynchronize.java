...
import com.dtac.sdp.subscription.status.query.CpaSubscriptionStatusQuery;
...
public class SubscriberSynchronize {
    ...
    private static final int _MAX_SYNC_ENTRY = 20;
    ...
    public void sendSyncStatus(ServiceElement ns, Subscriber[] msisdns, String register, String expired, String canceled)
            throws Exception {
        try {
            JAXBContext jc = JAXBContext.newInstance("com.dtac.sdp.subscription.status.query");
            Unmarshaller u = jc.createUnmarshaller();
            Marshaller m = jc.createMarshaller();
            StringBuilder xmlStr = new StringBuilder(
                    "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><cpa-subscription-status-query>"
                    + "        <txid>xxxxx</txid>"
                    + "        <authentication>"
                    + "                <user>myuser</user>"
                    ...
                    + "        </authentication>"
                    + "        <destination>"
                    + "                <productid>90114450001</productid>"
                    + "                <msisdn>xxxxxxxx</msisdn>"
                    + "        </destination>"
                    //                    + (register != null
                    //                    ? "        <start-date>20100216</start-date>"
                    //                    : "")
                    //                    + (canceled != null
                    //                    ? "        <canceled-date>20100216</canceled-date>"
                    //                    : "")
                    //                    + (expired != null
                    //                    ? "        <expired-date>20100216</expired-date>"
                    //                    : "")
                    + "</cpa-subscription-status-query>");

            CpaSubscriptionStatusQuery query =
                    (CpaSubscriptionStatusQuery) u.unmarshal(new StreamSource(new StringReader(xmlStr.toString())));

            query.getAuthentication().setUser(ns.oper_config.user);
            query.getAuthentication().setPassword(ns.oper_config.password);
            query.getDestination().setProductid(ns.bcast_srvc_id);

            // set TxID
            String cpid = ns.srvc_id.substring(0, 4);
            if (cpid.equals("4847")) {
                cpid = "088";
            } else if (cpid.equals("4677")) {
                cpid = "712";
            } else if (cpid.equals("4248")) {
                cpid = "248";
            } else {
                throw new Exception("CP[" + cpid + "] not supported!!");
            }

            String refid = String.format("0%s%s%03d", cpid, DatetimeUtil.getDateTime("ddHHmmss"), (int) (Math.random() * 1000));
            query.setTxid(refid);
...
//            }
            ...
            if ((msisdns != null) && (msisdns.length > 0)) {
                ...
                while (!subList.isEmpty()) {
                    ...
                    for (int i = 0; (i < _MAX_SYNC_ENTRY) && (!subList.isEmpty()); i++) {
    ...
    public int updateProfile(SubscriptionStatus subStatus, int srvc_main_id, SubscriberGroup.SUB_STATUS state) throws Exception {
        ...
        try {
            String sql = "UPDATE mmbr_dtac"
                    + " SET state=" + state.getId()
                    + "   , unregister_date=" + (state == SubscriberGroup.SUB_STATUS.UNREGISTER ? "CURDATE()" : "NULL")
                    + (register_date != null ? "   , register_date='" + register_date + "'" : "")
                    + (expired_date != null ? "   , expired_date='" + expired_date + "'" : "")
                    + " WHERE msisdn=?"
                    + "   AND srvc_main_id=?";
            ...
            log.log(Level.INFO, "updated subscription profile {0} record(s).", rows);
    ...
    public int updateSyncProfile(List<Subscriber> syncedList, List<SubscriptionStatus> subStatusList, int srvc_main_id) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < subStatusList.size(); i++) {
                ...
                Subscriber sub;
                try {
                    sub = new Subscriber(subStatus.getMsisdn(), srvc_main_id, CARRIER.DTAC_SDP);

                    if ((sub != null)
                            && (syncedList.remove(sub))) {
                        if (((subStatus.getStatus().equals("A")) || (subStatus.getStatus().equals("P")))
                                && (sub.getState() == SUB_STATUS.REGISTER.getId())
                                && (subStatus.getExpiredDate() != null)
                                && (subStatus.getExpiredDate().matches("^" + DatetimeUtil.print("yyyyMMdd", sub.getExpired_date()) + ".*"))) {
                            continue;
                        }

                        if ((!subStatus.getStatus().equals("A")) && (!subStatus.getStatus().equals("P"))
                                && (sub.getState() == SUB_STATUS.UNREGISTER.getId())) {
                            continue;
                        }
                    }
                } catch (Exception e) {
                ...
                try {
                    cp.getPreparedStatement().setInt(1, (subStatus.getStatus().equals("A")) || (subStatus.getStatus().equals("P")) 
                            ? SUB_STATUS.REGISTER.getId() 
                            : SUB_STATUS.UNREGISTER.getId());
            ...
            for (int i = 0; i < syncedList.size(); i++) {
                if (((Subscriber) syncedList.get(i)).getState() != SUB_STATUS.UNREGISTER.getId()) {
                    try {
                        log.log(Level.INFO, "unsub invalid result for {0} -> return: {1}", 
                                new Object[]{((Subscriber) syncedList.get(i)).getMsisdn(), ((Subscriber) syncedList.get(i)).doUnsub()});
                    } catch (Exception e) {
    ...
    public int createProfile(SubscriptionStatus subStatus, int srvc_main_id) throws Exception {
        ...
        DBPoolManager cp = new DBPoolManager();
        String sql =
                "  INSERT IGNORE INTO mmbr_" + CARRIER.DTAC_SDP.toString().toLowerCase()
                + "       (msisdn, srvc_main_id, ctnt_ctr, free_trial"
                + "     , rmdr_ctr, extd_ctr, non_expired, srvc_chrg_type_id, srvc_chrg_amnt"
                + "     , state, register_date, expired_date, balanced_date, unregister_date)"
                + " SELECT ?, s.srvc_main_id, s.ctnt_ctr, ?"
                + " , s.rmdr_ctr, s.rchg_ctr, 0, m.srvc_chrg_type_id, m.srvc_chrg_amnt"
                + " , ?, ?, ?, "
                + (subStatus.getLatestBilledDate() != null ? "'" + subStatus.getLatestBilledDate() + "'" : "NULL")
                + " , " + (subStatus.getStatus().equals("E") ? "'" + subStatus.getExpiredDate() + "'" : "NULL")
                + " FROM srvc_sub s"
                + " INNER JOIN srvc_main m"
    ...
    public List<SubscriptionStatus> readReply(String content, ServiceElement service) throws Exception {
        ...
        try {
            JAXBContext jc = JAXBContext.newInstance("com.dtac.sdp.subscription.status.reply");
